...
-- ============================================================
-- TDM V12.5 ADMIN / CHECKOUT / AFFILIATE FINAL PATCH
-- Run this block after the V12.4 migration. It is safe to rerun.
-- ============================================================

-- Product management: authenticated admin can fully manage products;
-- published products remain readable by customers.
...
-- Admin-configured distance for each delivery zone. Customers never enter KM.
alter table if exists public.shipping_zones add column if not exists distance_km numeric(10,2) not null default 0;

-- Public affiliate-link click logging. The referral link can be opened by a visitor
-- before they sign in; the function validates the affiliate code and records the click.
create or replace function public.tdm_log_affiliate_click(
...
declare
  ...
  click_id uuid;
begin
  select user_id into a from public.affiliates
  where affiliate_code=trim(p_affiliate_code) and status='active'
  limit 1;
  if a is null then raise exception 'Invalid affiliate code.'; end if;
  insert into public.affiliate_clicks(affiliate_user_id,affiliate_code,product_id)
  values(a,trim(p_affiliate_code),nullif(p_product_id,''))
  returning id into click_id;
  return click_id;
end;
$$;
revoke all on function public.tdm_log_affiliate_click(text,text) from public;
grant execute on function public.tdm_log_affiliate_click(text,text) to anon, authenticated;

-- Commission is calculated on the product value, not shipping/packaging/clearance.
create or replace function public.tdm_create_affiliate_commission() returns trigger
...
begin
 if new.payment_status='paid' and coalesce(old.payment_status,'') is distinct from 'paid' and new.affiliate_code is not null then
   ...
   if a is not null and not exists(select 1 from public.affiliate_commissions where order_id=new.id) then
     select coalesce(commission_rate,5) into rate from public.affiliate_products where product_id=coalesce(new.product_id,'') limit 1;
     amount=greatest(coalesce(new.total_amount,0)-coalesce(new.shipping_fee,0),0)*coalesce(rate,5)/100;
     insert into public.affiliate_commissions(affiliate_user_id,order_id,product_id,amount,commission_rate)
     values(a,new.id,new.product_id,amount,coalesce(rate,5));
     update public.affiliates set reward_balance=reward_balance+amount,lifetime_rewards=lifetime_rewards+amount where user_id=a;
   end if;
 end if; return new;
end; $$;

-- Rebuild the checkout function so shipping is charged once when a cart has multiple items.
create or replace function public.tdm_create_customer_order(
  p_items jsonb,
  p_shipping_address text,
  p_shipping_city text,
...
language plpgsql security definer set search_path=public as $$
declare
  item jsonb; new_id text; ids jsonb := '[]'::jsonb; uid uuid := auth.uid();
  qty integer; item_amount numeric; item_total numeric; row_no integer := 0;
begin
  if uid is null then raise exception 'You must be signed in to place an order.'; end if;
  ...
  if coalesce(trim(p_shipping_address),'')='' or coalesce(trim(p_shipping_city),'')='' or coalesce(trim(p_shipping_state),'')='' then raise exception 'Delivery address, city and state are required.'; end if;
  insert into public."Customers"(id,full_name,email,phone,delivery_address,updated_at)
  values(uid,coalesce(nullif(trim(p_customer_name),''),'Customer'),coalesce((select email from auth.users where id=uid),''),coalesce(p_customer_phone,''),p_shipping_address,now())
  on conflict(id) do update set full_name=excluded.full_name,phone=excluded.phone,delivery_address=excluded.delivery_address,updated_at=now();
  for item in select * from jsonb_array_elements(p_items) loop
    row_no := row_no + 1;
    qty := greatest(coalesce((item->>'quantity')::integer,1),1);
    item_amount := coalesce((item->>'item_total')::numeric,0);
    item_total := item_amount + case when row_no=1 then coalesce(p_shipping_fee,0) else 0 end;
    insert into public."Orders"(
      user_id,customer_id,product_id,product_name,quantity,total_amount,status,payment_status,
      shipping_address,shipping_city,shipping_state,shipping_country,shipping_distance_km,
      transportation_fee,packaging_fee,clearance_fee,shipping_fee,checkout_total,customer_name,customer_phone,affiliate_code
    ) values (
      uid,uid,nullif(item->>'product_id',''),item->>'product_name',qty,item_total,'Confirmed','unpaid',
      p_shipping_address,p_shipping_city,p_shipping_state,coalesce(nullif(trim(p_shipping_country),''),'Nigeria'),
      coalesce((item->>'distance_km')::numeric,0),coalesce(p_transportation_fee,0),coalesce(p_packaging_fee,0),
      coalesce(p_clearance_fee,0),case when row_no=1 then coalesce(p_shipping_fee,0) else 0 end,
      coalesce(p_checkout_total,0),coalesce(p_customer_name,''),coalesce(p_customer_phone,''),nullif(p_affiliate_code,'')
    ) returning id::text into new_id;
    ids := ids || jsonb_build_array(new_id);
  end loop;
  return ids;
...
-- Verification for the V12.5 patch.
select table_name from information_schema.tables where table_schema='public'
 and table_name in('customer_addresses','customer_notifications','shipping_settings','shipping_zones','affiliates','affiliate_products','affiliate_clicks','affiliate_commissions','affiliate_withdrawals');
select column_name,data_type from information_schema.columns where table_schema='public' and table_name='shipping_zones' and column_name='distance_km';
