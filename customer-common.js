...
(function(){
  ...
  window.tdmSaveCustomerProfile=async function(user,profile){
    ...
    const payload={full_name:profile.full_name,email:profile.email||user.email||'',phone:profile.phone||'',home_address:profile.home_address||'',delivery_address:profile.delivery_address||'',avatar_url:profile.avatar_url||'',updated_at:new Date().toISOString()};
    const existing=await c.from('Customers').select('id').eq('id',user.id).maybeSingle();
    if(existing.error)throw existing.error;
    let r;
    if(existing.data) r=await c.from('Customers').update(payload).eq('id',user.id);
    else r=await c.from('Customers').insert({id:user.id,...payload});
  ...
  function markCustomerNav(){const page=(location.pathname.split('/').pop()||'index.html').toLowerCase();document.querySelectorAll('.tdm-customer-nav a').forEach(a=>{const target=(a.getAttribute('href')||'').split('?')[0].split('/').pop().toLowerCase();a.classList.toggle('active',target===page || (page===''&&target==='index.html'));});}
document.addEventListener('DOMContentLoaded',()=>{tdmUpdateCartBadges();markCustomerNav();}); window.addEventListener('pageshow',()=>{tdmUpdateCartBadges();markCustomerNav();});
})();
